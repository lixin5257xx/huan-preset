...
let allNodeList:any[] = []; //全选用的数据
let allIdList:any[] = []; //全选用的数据
 ...
 * @param checkedKeys
 * @param option
 * 需要将对应的值 勾选掉 并且如果已有的值再次点击应该移除
 */
const handleChildCheckChange = (checkedKeys: any,options:any) => {
  checkedId.value = checkedKeys;
  checkedNode.value =  options.map((item,index)=>{
    return  item == null ? checkedNode.value[index] : item;
  })
  getShowChecked();
...
* 父级树形结构选中的回调
 */
const handleParentCheckChange = (checkedKeys: any, option: any) => {
  console.log("父级点击后","background:#f03df2; color:#fff");
  console.log(checkedKeys, option);
  option.forEach((item,index)=>{
    if (item[childrenParentIdKey] == 0) {
      const selectIndex = checkedKeys.findIndex(
          checkItem => checkItem == item[nodeKey]
      );
      checkedKeys.splice(selectIndex, 1);
    }
  })
...
const initChildTree = () => {
  if(checkedId.value.length){
    return parentTree.value.find(item=>{
    return item[childrenKey].find(childItem=>{
        return childItem[nodeKey] == checkedId.value[0]
      }) != undefined
    })[childrenKey]
  }else{
    return parentTree.value.length ? parentTree.value[0][childrenKey] || [] : [];
  }
}

// 初始化加载
const handleWacthChange = () => {
    ...
    parentTreeProvice.forEach((item: any) => {
        item.value = item[props.cascaderOption.parentLabel];
        item.disabled = disabled;
        flatList.value.push(item);
        if (item[childrenKey]) {
            item[childrenKey].forEach((itemChild: any) => {
                // 二级选中
                if (childChecked.includes(itemChild[nodeKey])) {
                    checkedNodeProvice.push(itemChild);
                }
                itemChild.disabled = disabled;
                itemChild.value = itemChild[props.cascaderOption.label];
                flatList.value.push(itemChild);
                allNodeList.push(itemChild);
                allIdList.push(itemChild[nodeKey]);
            });
        }
        // 一级选中
...
const checkAllChange = () => {
  checkedNode.value = cloneDeep(allNodeList);
    ...
    <div class="cascader-tree">
      <div class="search-title">
          <n-input
              ...
              :disabled="cascaderOption.disabled"
              clearable
          />
      </div>
        ...
        <div class="selectTree">
          <div class="parentTree">
            <div class="tree-title">
                ...
                <div class="title-right">
                  <span
                      class="checkedTree__title__clear checkAll"
                      :disabled="cascaderOption.disabled"
            ...
            <n-tree
                ...
                @update:selected-keys="handleParentClick"
                @update:checked-keys="handleParentCheckChange"
                :key-field="cascaderOption.parentIdKey"
                :label-field="cascaderOption.parentLabel"
          ...
          <div class="childTree" v-show="childTree.length > 0">
            ...
            <n-tree
                ...
                :checked-keys="checkedId"
                :disabled="cascaderOption.disabled"
                :key-field="cascaderOption.childrenIdKey"
                :label-field="cascaderOption.childrenLabel"
                @update:checked-keys="handleChildCheckChange"
        ...
        <div class="checkedTree" v-show="cascaderOption.showTag">
          <div class="tree-title">
            <div class="search-title-right">
              ...
              <span
                  v-if="!cascaderOption.disabled"
                  @click="handleClickClear"
                  class="checkedTree__title__clear"
              >清空</span>
            </div>
          </div>
          <div class="checkedTree__tagBox">
            <n-tag
                :bordered="false"
                v-for="item in showChecked"
                :key="item[cascaderOption.nodeKey]"
                :closable="!cascaderOption.disabled"
